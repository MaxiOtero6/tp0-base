...
-   Server health-check script del ejercicio 3

## Ejercicio 4

Para este ejercicio se agrego tanto del lado del servidor como del cliente el handling de la signal SIGTERM para terminar la ejecucion de manera gracefully

En el lado del cliente mediante un canal de signals, al recibir SIGTERM se cierra la conexion con el servidor y se corta la ejecucion cerrando el canal Client.done.

Por el lado del servidor, al recibir SIGTERM, cierra su socket y sale del server loop para no intentar aceptar mas conexiones (ya que el socket se encuentra cerrado).
