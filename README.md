...
Para este ejercicio se cambio el comportamiento tanto del servidor como del cliente, en lugar de ser un echo server, ahora el servidor espera recibir una apuesta por conexion de cada agencia (cliente), guardarla en un archivo .csv y confirmarle al cliente que recibio la apuesta.

Tanto del lado del servidor como del cliente, se implemento la clase / struct Socket el cual es un wrapper de un socket tcp el cual asegura que no se produzcan short reads / writes.

En el lado cliente se reciben los datos de la apuesta a enviar mediante variables de entorno, generando con ellas un DTO BetPacket el cual se serializa segun el protocolo propuesto.
